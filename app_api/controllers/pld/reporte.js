...
module.exports.insertOperation = async (req, res) => {
    ...
    const descripcionOperacion = req.body.descripcionOperacion
    const razones = req.body.razones
    ...
    if (!organoSupervisor || organoSupervisor.length != 6) {
        sendJSONresponse(res, 404, { message: 'Ingresa un Órgano Supervisor válido. El campo debe ser de' })
    }

    // Check columna 5 - Clave Sujeto Obligado
    if (!claveSujetoObligado || !(claveSujetoObligado.length == 7 || claveSujetoObligado.length == 8)) {
        sendJSONresponse(res, 404, { message: 'Ingresa una Clave o Número de Registro del Sujeto Obligado válida' })
        return
    }
    ...
    if (tipoReporte == 1 || ((tipoReporte == 2 || tipoReporte == 3) && (!consecutivoCuentas || consecutivoCuentas == '00'))) {
        ...
        if (!nacionalidad || (nacionalidad != 1 || nacionalidad != 2)) {
        ...
        if (!tipoPersona || (tipoPersona != 1 || tipoPersona != 2)) {
        ...
        if ((!rfc && (!curp || !fechaNacimiento)) || rfc.length > 13) {
        ...
        if ((!curp && (!rfc || !fechaNacimiento)) || curp.length != 18) {
        ...
        if ((!fechaNacimiento && (!rfc || !curp)) || fechaNacimiento.length != 8) {
